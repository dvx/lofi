...
class Controls extends React.Component<any, any> {
  ...
  async toggleShuffle() {
    if (this.props.parent.getShuffleState()) {
    ...
    // Assume original state is correct and make UI a bit snappier
    this.props.parent.toggleShuffleInstant();
  }
  ...
  render() {
    return (
      <div className='controls centered'>
        ...
        <a onClick={this.toggleShuffle.bind(this)} className={'control-btn ' +(this.props.parent.getShuffleState() ? 'enabled' : 'disabled')+' not-draggable secondary-control' } ><i className="fa fa-random not-draggable" ></i></a>
        </p>
      </div>
    );
  }
